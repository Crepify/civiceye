...
- AI: On-device (Transformers.js, Xenova/yolos-tiny) = first, free & private (only trusted
  when it maps to a civic category at ≥ 0.35 confidence); Roboflow (branded "CivicLENS AI")
  object detection = PRIMARY cloud engine; Hugging Face Inference API (facebook/detr-resnet-50)
  = cloud backup; built-in mock estimate = last resort. Groq was REMOVED from the chain.
...
flagService, geoService, geocodeService, mapService, reportService, reviewService,
roboflowService, onDeviceService, huggingfaceService, syncService), styles/index.css, types/index.ts, utils/ (cn, format, geo,
image, auth, download).
...
- AI pipeline: runImageAnalysis → on-device (Transformers.js) → Roboflow (via proxy
  /api/roboflow or VITE_ROBOFLOW_PROXY_URL Worker) → Hugging Face → mock. Photos compressed
  to 768px JPEG ~72 before sending. Roboflow
...
- VITE_ROBOFLOW_API_KEY = <ROBOFLOW_API_KEY> (SHARED IN CHAT — ROTATE).
...
- GitHub secret scanning BLOCKED a push because a handoff doc contained real API keys — they
  were redacted in the repo copy (Groq is no longer part of the stack). There may still be
...
2. ROTATE the Roboflow API key (shared in chat) and revoke the old Groq key too (exposed in
   chat, even though Groq is no longer used); update Vercel env.
